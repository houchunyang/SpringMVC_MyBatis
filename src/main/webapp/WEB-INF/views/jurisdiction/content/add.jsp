...
<div id="jurisdictionAddPanel">
	<div class="headline">
		<h2>
			...
			<spring:message code="menu.AddJurisdiction" />
		</c:if>
			<c:if test="${jurisdiction.id != 0 }">
			<spring:message code="menu.UpdateJurisdiction" />
		</c:if>
		</h2>
	</div>
	<form class="real-content-form form" name="jurisdiction" action="${ctx }/jurisdiction/content/save" method="post">
		...
		<div class="form-group">
			<label class="input-label"><spring:message code="menu.jurisdictionName" /></label> <input name="name" class="input-content" type="text" placeholder="请输入名称" value="${jurisdiction.name }">
		</div>
	...
	<div class="bottom-menu-panel">
		<button class="btn btn-bg save-jurisdiction-btn"><spring:message code="menu.save" /></button>
